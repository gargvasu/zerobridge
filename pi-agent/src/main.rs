...
async fn main() {
    ...
    match bridge.request(Request::GetCursor).await {
        Ok(serial::protocol::Response::CursorPos { x, y }) => println!("✅ Cursor: ({}, {})", x, y),
        Ok(r) => eprintln!("Unexpected: {:?}", r),
        Err(e) => eprintln!("❌ {}", e),
    }
    ...
    match bridge.request(Request::GetActiveApp).await {
        Ok(serial::protocol::Response::ActiveApp { name, .. }) => println!("✅ App: {}", name),
        Ok(r) => eprintln!("Unexpected: {:?}", r),
        Err(e) => eprintln!("❌ {}", e),
    }
    ...
    println!(
        "  SSH USB:  {}",
        if status.ssh_usb_healthy { "✅" } else { "❌" }
    );
    println!(
        "  SSH WiFi: {}",
        if status.ssh_wifi_healthy {
            "✅"
        } else {
            "❌"
        }
