...
info "Stopping pi-agent daemon on Pi (if running)..."
ssh "$PI_USER@$PI_HOST" "sudo systemctl stop pi-agent 2>/dev/null || true; sudo killall pi-agent 2>/dev/null || true; sleep 0.5"
ok "pi-agent stopped"
...
ok "Binary staged at ~/pi-agent-new (previous → ~/pi-agent-new.bak)"
...
echo ""
echo "═══════════════════════════════════════"
ok "Deploy staged — pi-agent stopped, ready to test"
echo ""
...
echo ""
echo "  2. Run test daemon on a separate socket:"
